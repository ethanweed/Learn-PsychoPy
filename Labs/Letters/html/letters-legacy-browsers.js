...
let expName = 'letters';  // from the Builder filename that created this script
let expInfo = {'participant': '', 'age': '', 'handedness (right/left)': ''};
...
function instrRoutineEachFrame(trials) {
  return function () {
    ...
    if (key_resp_2.status === PsychoJS.Status.STARTED) {
      let theseKeys = key_resp_2.getKeys({keyList: ['down'], waitRelease: false});
...
function trialsLoopBegin(thisScheduler) {
  ...
  trials = new TrialHandler({
    psychoJS: psychoJS,
    nReps: 100, method: TrialHandler.Method.FULLRANDOM,
    extraInfo: expInfo, originPath: undefined,
    trialList: 'stims.xlsx',
...
function trialRoutineBegin(trials) {
  return function () {
    ...
    // keep track of which components have finished
    trialComponents = [];
    trialComponents.push(letter_left);
    trialComponents.push(letter_right);
...
function trialRoutineEachFrame(trials) {
  return function () {
    ...
    // *letter_left* updates
    if (t >= 0.1 && letter_left.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
    ...
    if (t >= 0.1 && letter_right.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
...
var doneComponents;
function doneRoutineBegin(trials) {
  return function () {
...
var frameRemains;
function doneRoutineEachFrame(trials) {
  return function () {
    ...
    // *text_done* updates
    if (t >= 0.0 && text_done.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      text_done.tStart = t;  // (not accounting for frame time here)
    ...
    if (text_done.status === PsychoJS.Status.STARTED && t >= frameRemains) {
...
function doneRoutineEnd(trials) {
  return function () {
    //------Ending Routine 'done'-------
    doneComponents.forEach( function(thisComponent) {
      if (typeof thisComponent.setAutoDraw === 'function') {
