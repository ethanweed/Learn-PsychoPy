...
expName = 'letters'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/ethan/Documents/GitHub/Learn-PsychoPy/Labs/letters/letters_lastrun.py',
...
text = visual.TextStim(win=win, name='text',
    text='Press left arrow if the A is on the left.\n\nPress right arrow if the A is on the right.\n\nPress down arrow to begin...',
    font='Arial',
...
trials = data.TrialHandler(nReps=100, method='fullRandom', 
...
for thisTrial in trials:
    ...
    continueRoutine = True
    # update component parameters for each repeat
    letter_left.setText(left)
    letter_right.setText(right)
    key_resp.keys = []
    key_resp.rt = []
    _key_resp_allKeys = []
    # keep track of which components have finished
    trialComponents = [letter_left, letter_right, key_resp, ISI]
    ...
    while continueRoutine:
        ...
        # *letter_left* updates
        if letter_left.status == NOT_STARTED and tThisFlip >= 0.1-frameTolerance:
            # keep track of start time/frame for later
        ...
        if letter_right.status == NOT_STARTED and tThisFlip >= 0.1-frameTolerance:
            # keep track of start time/frame for later
    ...
    for thisComponent in trialComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    trials.addData('letter_left.started', letter_left.tStartRefresh)
    trials.addData('letter_left.stopped', letter_left.tStopRefresh)
    trials.addData('letter_right.started', letter_right.tStartRefresh)
    trials.addData('letter_right.stopped', letter_right.tStopRefresh)
    # check responses
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    if text_done.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > text_done.tStartRefresh + 4-frameTolerance:
            # keep track of stop time/frame for later
            text_done.tStop = t  # not accounting for scr refresh
            text_done.frameNStop = frameN  # exact frame index
            win.timeOnFlip(text_done, 'tStopRefresh')  # time at next scr refresh
            text_done.setAutoDraw(False)
